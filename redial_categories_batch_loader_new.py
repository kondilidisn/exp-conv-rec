...
class DialogueBatchLoader4Categories(object):
    def __init__(self, sources, batch_size,
        ...
        self.preprocess_ratings_into_tokenized_lists()


        self.preprocess_for_categories()

        

    def load_category_vectors(self, filename = "movie_details.csv"):
    ...
    def preprocess_for_categories(self):
        ...
        for subset in ["train", "valid", "test"]:
            ...
            for i in range(len(self.ratings_token_ids_lists_data[subset])):
                ...
                sample = self.ratings_token_ids_lists_data[subset][i]
    ...
    def preprocess_ratings_into_tokenized_lists(self, smooth_targets = False):
        ...
        for subset in self.ratings_token_ids_lists_data:
            ...
            for sample in self.ratings_data[subset]:
                ...
                if len(sample) > 2:
                    self.ratings_token_ids_lists_data[subset].append(([(movie_id, self.rating_targets["liked"] if sample[movie_id] > 0.5 else self.rating_targets["disliked"]) for movie_id in sample]))
                else:
