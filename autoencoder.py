...
class EvaluationClass(nn.Module):
	...
	def evaluate_output(self, input, target, output, eval_ranking = True):
	...
	def evaluate_output_multithreaded(self, input, target, output, num_of_workers, eval_ranking = True):
		...
		if num_of_workers == 0 or eval_ranking == False:
			self.evaluate_output(input, target, output, eval_ranking = eval_ranking)
...
class Embedder(EvaluationClass):
	"""



	"""
	def __init__(self, n_movies, layer_sizes = [1000], num_of_categories = 19, task="categories", load_model_path = ""):
		super(Embedder, self).__init__()


		self.encoder = nn.Linear(in_features=n_movies, out_features=num_of_categories, bias = False)

		# self.decoder = nn.Linear(in_features=n_movies, out_features=num_of_categories, bias = False)
	...
	def forward(self, input):
		...
		cpd = torch.softmax(self.encoder(input), dim = -1) 


		if self.task == "ratings":

			# print(cpd.size(), self.encoder.weight.size())

			item_scores = torch.sigmoid(torch.matmul(cpd, self.encoder.weight))
			# exit()
		...
		for i in tqdm(range(number_of_batches)):
			...
			if args.task == "categories":
				...
				if args.dataset == "redial":
					...
					self.evaluate_output_multithreaded(input = input_vectors, target = rating_targets, output = output, num_of_workers = args.num_of_workers, eval_ranking = True)
				else:
	...
	def __init__(self, number_of_movies, num_of_categories, layer_sizes=[1000], decoder_mode = "symmetric", load_model_path = "", task = "categories", transfer_learning = False):
		...
		if load_model_path != "":
			...
			if "ratings" in lastly_modified_training_filename:
				...
				for i in range( len(decoder_layer_sizes) -1):
					self.decoder_layers.append( nn.Linear(in_features=decoder_layer_sizes[i], out_features=decoder_layer_sizes[i+1], bias=False))
